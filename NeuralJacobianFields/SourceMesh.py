...
class SourceMesh:
    ...
    def __init_from_mesh_data(self):
        ...
        m_vertices = self.mesh_processor.get_vertices() # if eyeremoved_index is None else self.mesh_processor.get_vertices()[eyeremoved_index];
        print("m_vertices shape: ", m_vertices.shape);
        self.source_vertices = torch.from_numpy(m_vertices).type(
            self.__ttype)
        ...
        print("Ellapsed load source mesh: --------------------------------")

    #we wanna load the mesh data from the vert and face directly, not from the source_dir
    def load(self, source_v=None, source_f=None):
        # mesh_data = SourceMeshData.SourceMeshData.meshprocessor_from_file(self.source_dir)
        if source_v is not None and source_f is not None:   #we could go with this, but need the directory folder rather than the mesh
            self.mesh_processor = MeshProcessor.MeshProcessor.meshprocessor_from_array(source_v,source_f, os.path.dirname(self.source_dir), self.__ttype, cpuonly=self.cpuonly, load_wks_samples=self.__use_wks, load_wks_centroids=self.__use_wks)
            print("Initializing Mesh for NJF -------------------------------")
        else:
    ...
    #def __del__(self):
        #del self.poisson;
    #    print("delete source mesh object");
    def pin_memory(self):
